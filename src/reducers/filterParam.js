  ...
  switch (action.type) {
        ...
        if (action.payload[i].type === 'slider') {
          state = {
            ...
            [action.payload[i].display_name]: {
              ...
              data: {
                min: action.payload[i].slider_min_value,
                max: action.payload[i].slider_max_value
              }
            }
          };
    ...
    case SET_FILTERED_LIST_PARAM: {
      if (state[action.filterData.display_name].data.includes(action.payload)) {
        state[action.filterData.display_name].data = state[
          action.filterData.display_name
