...
const FilterSlider = ({ reset, filtering, filterData }) => {
  ...
  useEffect(() => {
    ...
    store.dispatch(
      setFilteredSliderParam(
        { min: filterData.slider_min_value, max: filterData.slider_max_value },
  ...
  const onChangeInputMin = (minValue) => {
    ...
    store.dispatch(
      setFilteredSliderParam({ min: minValue, max: maxValue }, filterData.display_name)
    );
    filtering();
  };
  ...
  const cleanDataClass = cn('filter__clean-data', {
    close:
      isCleanSliderData ||
      (filterData.slider_min_value === minValue && filterData.slider_max_value === maxValue)
  });
